...
class MagnedAI {
  ...
  _quiz(lower) {
    ...
    let r = <div style="font-family:sans-serif"><div style="font-size:1.1rem;font-weight:bold;margin-bottom:15px;color:var(--text-1)">?? Quiz � ${topic.charAt(0).toUpperCase() + topic.slice(1)}</div>;
    
    selected.forEach((q, i) => {
      r += <div style="margin-bottom:20px"><div style="font-weight:600;color:var(--text-1);margin-bottom:8px">${i + 1}. ${q.q}</div>;
      q.opts.forEach((opt, j) => {
        if (j === q.correct) {
          r += <div style="color:var(--mint); margin-top:4px"><i class="fas fa-check-circle"></i> ${String.fromCharCode(65 + j)}) ${opt}</div>;
        } else {
          r += <div style="color:var(--text-2); margin-top:4px"><i class="far fa-circle"></i> ${String.fromCharCode(65 + j)}) ${opt}</div>;
        }
      });
      r += <div style="color:var(--amber); font-style:italic; margin-top:10px; font-size:0.9em; border-left:3px solid var(--amber); padding-left:10px">Explica��o: ${q.explanation}</div></div>;
    });
    ...
    r += <div style="border:1px solid var(--border); border-radius:16px; padding:20px; margin-top:20px; text-align:center; background:rgba(0,0,0,0.2)">
            <div style="display:flex; justify-content:center; gap:15px; margin-bottom:20px; position:relative; height:180px">
                <div style="position:absolute; background:var(--bg-elevated); padding:20px; border-radius:12px; border:1px solid var(--amber); width:160px; height:150px; transform:rotate(-5deg); left:calc(50% - 90px); box-shadow:0 8px 24px rgba(0,0,0,0.3); display:flex; flex-direction:column; justify-content:center; align-items:center; z-index:2">
                    ...
                    <div style="font-size:0.8rem; color:var(--text-2); margin-bottom:15px; text-align:center">${selected[0].q.substring(0, 45)}...</div>
                    <button class="btn-s" style="padding:6px 12px; font-size:0.75rem; border:1px solid var(--amber); background:transparent; color:var(--amber)">Virar Cart�o</button>
                </div>
                <div style="position:absolute; background:var(--bg-elevated); padding:20px; border-radius:12px; border:1px solid var(--border); width:160px; height:150px; transform:rotate(5deg); left:calc(50% - 70px); box-shadow:0 8px 24px rgba(0,0,0,0.3); display:flex; flex-direction:column; justify-content:center; align-items:center; opacity:0.6; z-index:1">
                    ...
                    <div style="font-size:0.8rem; color:var(--text-2); text-align:center">${selected[0].explanation.substring(0, 35)}...</div>
                </div>
            </div>
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; max-width:260px; margin:0 auto; margin-bottom:15px;">
                ...
                <button class="btn-s" style="background:transparent; border:1px solid var(--border); padding:6px 12px; font-size:0.8rem">Pr�ximo ></button>
            </div>
